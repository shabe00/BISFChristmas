...
class PlayState extends MusicBeatState
{
	...
	public var orion:FlxSprite;
	public var fling:FlxSprite;
	public var roof2:FlxSprite;
	...
	var allowedToSing:Bool = false;
	var testicles:Bool = false;
	...
	override public function create()
	{
		...
		dad = new Character(400, -50, 'orion');
		add(dad);
		remove(dad);
			...
			case 'SFmall':
				{
					...
					var bg:FlxSprite = new FlxSprite(-1100, -700).loadGraphic(Paths.image('Mwall', 'shared'));
					bg.scale.set(0.8, 0.8);
					bg.antialiasing = true;
					bg.active = false;
					add(bg);

					var rooftex = Paths.getSparrowAtlas('Roof', 'shared');
					var roof2tex = Paths.getSparrowAtlas('snow_roof_2', 'shared');
					var boomtex = Paths.getSparrowAtlas('boombox', 'shared');
					var oniontex = Paths.getSparrowAtlas('OnionLand', 'shared');
					var flingtex = Paths.getSparrowAtlas('fling', 'shared');
					var boppertex = Paths.getSparrowAtlas('SFXBop', 'shared');

					var roof = new FlxSprite(-650, -725).loadGraphic(Paths.image('Roof', 'shared'));
					roof.frames = rooftex;
					roof.animation.addByPrefix('snow', "snow roof0", 24);
					roof.animation.play('snow');
					roof.antialiasing = true;
					add(roof);

					roof2 = new FlxSprite(-650, -725).loadGraphic(Paths.image('snow_roof_2', 'shared'));
					roof2.frames = roof2tex;
					roof2.animation.addByPrefix('snow', "snow roof 2", 24);
					roof2.animation.play('snow');
					roof2.antialiasing = true;
					roof2.visible = false;
					add(roof2);

					var dec:FlxSprite = new FlxSprite(-725, -250).loadGraphic(Paths.image('decorations', 'shared'));
					dec.scale.set(0.9, 0.9);
					dec.antialiasing = true;
					dec.active = false;
					add(dec);

					var boppers = new FlxSprite(-850, 0).loadGraphic(Paths.image('SFXBop', 'shared'));
					boppers.frames = boppertex;
					boppers.scale.set(0.8, 0.8);
					boppers.animation.addByPrefix('idle', "SFXBoppers0", 24);
					boppers.animation.play('idle');
					boppers.antialiasing = true;
					add(boppers);

					boombox = new FlxSprite(230, 350).loadGraphic(Paths.image('boombox', 'shared'));
					boombox.frames = boomtex;
					boombox.animation.addByPrefix('idle', "boombox0", 24);
					boombox.animation.play('idle');
					boombox.antialiasing = true;
					boombox.visible = false;
					add(boombox);

					orion = new FlxSprite(0, -980).loadGraphic(Paths.image('OnionLand', 'shared'));
					orion.frames = oniontex;
					orion.animation.addByPrefix('idle', "Onion_Landing0", 24, false);
					orion.antialiasing = true;
					orion.visible = false;
					add(orion);
					
					fling = new FlxSprite(-450, -100).loadGraphic(Paths.image('fling', 'shared'));
					fling.frames = flingtex;
					fling.animation.addByPrefix('idle', "fling0", 24, false);
					fling.antialiasing = true;
					fling.animation.play('idle');
					fling.visible = false;
					add(fling);

					santa = new FlxSprite(-230, 100);
					santa.frames = Paths.getSparrowAtlas('christmas/santa', 'week5');
					santa.animation.addByPrefix('idle', 'santa idle in fear', 24, false);
					santa.antialiasing = true;
					santa.visible = false;
					roof.animation.play('idle');
					add(santa);
				}
			case 'stage':
				{
		...
		gf = new Character(225, 0, curGf);

		dad = new Character(200, 100, SONG.player2);

		dad2 = new Character(100, 100, 'dusty');
		...
		boyfriend = new Boyfriend(200, 260, SONG.player1);
		...
		redScreen = new FlxSprite(0, 0).makeGraphic(Std.int(FlxG.width * 2), Std.int(FlxG.height * 2), FlxColor.WHITE);
		redScreen.alpha = 0;
		add(redScreen);
			...
			if (camFollow.x != dad.getMidpoint().x + 150 && !PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
			{
				...
				switch (dad.curCharacter)
				{
					...
					case 'santa':
						...
						FlxTween.tween(FlxG.camera, {zoom: 0.5}, (Conductor.stepCrochet * 4 / 1000), {ease: FlxEase.quadInOut});
					case 'orion':
	...
	*/
	...
	override function stepHit()
	{
		...
		if (curStep == 270)
			{
				FlxG.sound.play(Paths.sound('holyshitorion'));
			}
		if (curStep == 272)
			{
				remove(gf);
				...
				orion.animation.play('idle', false);
				fling.animation.play('idle', false);
			}
		if (curStep == 276)
			{
				fling.visible = false;
		...
		if(curStep == 610 || curStep == 285 || curStep == 866)
			{
				iconP2.animation.play('orion');
			}
		if(curStep == 672 || curStep == 482 || curStep == 896)
			{
				iconP2.animation.play('dusty');
			}
		if(curStep == 736)
			{
				iconP2.animation.play('donion');
			}
		if (curStep == 480)
			{
				dad2.visible = true;
				redScreen.alpha = 0.50;
			}
		if (curStep == 481)
			{
				redScreen.alpha = 0.50;
				new FlxTimer().start(0.1, function(tmr:FlxTimer)
				{
					FlxTween.tween(redScreen,{alpha: 0},0.1);
				});
			}
		if (curStep == 480 || curStep == 672 || curStep == 896)
		{
			allowedToSing = true;
			testicles = false;
		}
	...
	override function beatHit()
	{
		...
		if (SONG.notes[Math.floor(curStep / 16)] != null)
		{
			...
			if (SONG.notes[Math.floor(curStep / 16)].mustHitSection && dad.curCharacter != 'gf')
				dad.dance();
			if (SONG.notes[Math.floor(curStep / 16)].mustHitSection && dad2.curCharacter != 'gf')
				dad2.dance();
		}
